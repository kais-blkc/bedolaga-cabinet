...
export default function PurchaseSheet({
  ...
  const sheetTitle =
    mode === 'change'
      ...
      : t('subscription.purchase', 'Купить подписку');
    ...
    <Sheet open={open} onOpenChange={(o) => !o && handleClose()}>
      <SheetContent>
        ...
        <div className="mt-4 space-y-4">
          ...
          ) : optionsError || (!purchaseOptions && !optionsLoading) ? (
            <div className="rounded-2xl border border-dark-700/50 bg-dark-800/60 p-6 text-center">
              <p className="mb-4 text-dark-400">
                {t('subscription.loadError', 'Не удалось загрузить варианты подписки')}
              </p>
              <button
                onClick={() => refetchOptions()}
                className="rounded-xl bg-accent-500 px-6 py-2 text-sm font-medium text-dark-950 transition-colors hover:bg-accent-600"
              >
                {t('common.retry')}
              </button>
            </div>
              ...
              {isTariffsMode && tariffs.length > 0 && (
                <div className="space-y-4">
                  ...
                  {switchTariffId && (
                    <div
                      ref={switchModalRef}
                      className="space-y-4 rounded-xl border border-dark-700/50 bg-dark-800/50 p-5"
                    >
                      ...
                      {switchPreviewLoading ? (
                        ...
                        switchPreview &&
                        (() => {
                          const targetTariff = tariffs.find(
                            (tariff) => tariff.id === switchTariffId,
                          );
                          const dailyPrice =
                            targetTariff?.daily_price_kopeks ??
                            targetTariff?.price_per_day_kopeks ??
                            0;
                          ...
                          return (
                            <>
                              ...
                              <button
                                ...
                                disabled={
                                  switchTariffMutation.isPending || !switchPreview.can_switch
                                }
                                className="w-full rounded-2xl bg-accent-500 py-3 text-sm font-semibold text-dark-950 transition-opacity active:opacity-80 disabled:opacity-50"
                              >
                  ...
                  {!showTariffPurchase ? (
                          ...
                          <div>
                            <div className="text-sm font-medium text-success-400">
                              {t('subscription.promoGroup.yourGroup', {
                                name: tariffs.find((tariff) => tariff.promo_group_name)
                                  ?.promo_group_name,
                              })}
                            </div>
                          ...
                          <div className="rounded-2xl border border-dark-700/50 bg-dark-800/60 p-6 text-center">
                            ...
                            <p className="text-sm text-dark-400">
                              {t(
                                ...
                                'Вы уже приобрели все доступные тарифы.',
                              )}
                            </p>
                          </div>
                        )}
                        ...
                        {[...tariffs]
                          ...
                          .map((tariff) => {
                            ...
                            return (
                              <div
                                key={tariff.id}
                                className={`rounded-2xl border p-4 ${isCurrentTariff ? 'border-accent-500/50 bg-accent-500/5' : 'border-dark-700/50 bg-dark-800/60'}`}
                              >
                                ...
                                <div className="flex flex-wrap gap-3 text-sm text-dark-400">
                                  <span>{tariff.traffic_limit_label}</span>
                                  <span>
                                    {tariff.device_limit === 0
                                      ? '∞ ' + t('subscription.devices', { count: 0 })
                                ...
                                <div className="mt-3 border-t border-dark-700/50 pt-3 text-sm text-dark-400">
                                  {(() => {
                                    ...
                                    const originalDailyPrice =
                                      tariff.original_daily_price_kopeks || 0;
                                  ...
                                  {isCurrentTariff ? (
                                    subscription?.is_daily ? (
                                      ...
                                      <button
                                        ...
                                        className="flex-1 rounded-xl bg-accent-500 py-2 text-sm font-semibold text-dark-950 transition-opacity active:opacity-80"
                                      >
                                    ...
                                    <button
                                      ...
                                      className="flex-1 rounded-xl border border-dark-600 bg-dark-800 py-2 text-sm font-medium text-dark-200 transition-colors active:bg-dark-700"
                                    >
                                      {t('subscription.switchTariff.switch')}
                                    </button>
                                  ) : (
                    ...
                    selectedTariff && (
                      ...
                      <div ref={tariffPurchaseRef} className="space-y-4">
                        <div className="flex items-center justify-between">
                                    ...
                                    return (
                                      <button
                                        ...
                                        className={`relative rounded-xl border p-4 text-left transition-all ${
                                          selectedTariffPeriod?.days === period.days &&
                                          !useCustomDays
                                ...
                                !(
                                  ...
                                  <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-center">
                                    ...
                                    <button
                                      ...
                                      className="mt-3 rounded-xl border border-dark-600 bg-dark-800 px-4 py-2 text-sm text-dark-200"
                                    >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {!showPurchaseForm && (
                      <button
                        ...
                        className="rounded-xl bg-accent-500 px-4 py-2 text-sm font-semibold text-dark-950 transition-opacity active:opacity-80"
                      >
                        {subscription && !subscription.is_trial
                          ? t('subscription.extend')
                          : t('subscription.getSubscription')}
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (availableServers.length === 1) {
                                    ...
                                    setSelectedServers(
                                      period.servers.selected.filter((uuid) =>
                                        availUuids.has(uuid),
                                      ),
                                    );
                                  }
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {selectedPeriod.servers.options
                            .filter((server) => {
                              ...
                              if (
                                subscription?.is_trial &&
                                server.name.toLowerCase().includes('trial')
                              ) {
                                return false;
                              }
                              return true;
                        ...
                        <div className="flex flex-col items-center py-6">
                          <div className="flex items-center gap-6">
                            <button
                              ...
                              className="flex h-14 w-14 items-center justify-center rounded-xl border border-dark-600 bg-dark-800 text-2xl text-dark-200 disabled:opacity-40"
                            >
                              -
                            </button>
                      ...
                      {currentStep === 'confirm' && (
                        <div>
                          {previewLoading ? (
                            ...
                            <div className="space-y-4 rounded-xl border border-dark-700/50 bg-dark-800/50 p-5">
                                ...
                                return (
                                  <div className="flex items-center justify-between border-t border-dark-700/50 pt-4">
                                    ...
                                    <div className="text-right">
                                      ...
                                      {promoTotal.original &&
                                        promoTotal.original > promoTotal.price && (
                                          <div className="text-sm text-dark-500 line-through">
                      ...
                      <div className="flex gap-3 border-t border-dark-800/50 pt-4">
                        {!isFirstStep && (
                          <button
                            ...
                            className="flex-1 rounded-xl border border-dark-600 bg-dark-800 py-3 text-sm font-medium text-dark-200"
                          >
                            {t('common.back')}
                          </button>
                        )}
                        ...
                        {isFirstStep && (
                          <button
                            ...
                            className="rounded-xl border border-dark-600 bg-dark-800 px-4 py-3 text-sm font-medium text-dark-200"
                          >
                            {t('common.cancel')}
                          </button>
                        )}
                        ...
                        {!isLastStep ? (
                          <button
                            ...
                            className="flex-1 rounded-xl bg-accent-500 py-3 text-sm font-semibold text-dark-950 transition-opacity active:opacity-80 disabled:opacity-50"
                          >
                            {t('common.next')}
                          </button>
                        ) : (
